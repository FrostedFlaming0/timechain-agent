...
knows nothing about prompts, agents glue them together, `run.py` configures
everything.** If your change blurs those lines, expect questions.
...
| Retrieval | `retrieval.py` | embeddings, similarity, salience |
| Agent | `agent.py` | turn loop, reflection, revision, prompt formatting |
...
- **Retrieval improvements** — better salience defaults, time-window
  filters, smarter context budgeting. These need benchmarks or at least a
  before/after example showing why the change is an improvement.
...
2. Add a default salience weight in `Retriever.DEFAULT_SALIENCE` if the
   type should bias retrieval, and a retention priority in
   `Agent._RETENTION_PRIORITY` if it should survive context-budget pressure.
3. Update `view_chain.py`'s `fmt_content` if the type has non-obvious
   content shape.
4. Document the type in `ARCHITECTURE.md`'s record-types table.
5. Add a test that round-trips the type through the chain and retrieval.
...
- **Module docstrings explain what the file is for and how it fits in.**
  Look at the top of `file_ingest.py` or `retrieval.py` for examples.
  New modules should do the same.
...
- **Dataclasses for structured returns.** See `IngestResult`, `RetrievalHit`,
  `Record`. Tuples are fine for two-element returns; past that, use a
  dataclass.
...
- Keep tests deterministic. The `HashingEmbedder` (trigram-bag) is used in
  tests precisely because it has no model dependency and gives stable
  results.
...
new file extension supported — update `README.md` and `ARCHITECTURE.md` in
the same PR. Documentation that lags behind the code is a bug.
