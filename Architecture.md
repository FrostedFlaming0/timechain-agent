...
A guide to the five files that make up the timechain agent, what each is responsible for, and how they fit together.
...
│                    records to feed into LLM context     │
├─────────────────────────────────────────────────────────┤
...
└─────────────────────────────────────────────────────────┘
  ...
  llm_clients.py    PLUGGABLE — LLM provider clients,
                    sits beside the stack, used by run.py
```
...
`Retriever` is the query interface. It combines vector search with structural access patterns. Methods:
- `hybrid(query, k, type_filter, recency_weight)` — semantic search with optional filters and a recency boost.
...
**Responsibility:** orchestrate a turn. Take user input, retrieve relevant context, format a prompt, call the LLM, write everything back to the chain.

**Knows about:** the chain, the retriever, the LLM (as a callable).
**Does not know about:** which LLM provider, embedding internals, prompts (it knows there *is* a prompt, but the format is local).

**Key components:**

`Agent` class with two main methods:
...
- `turn(user_input, retrieve_k)` — the heart of the loop:
  ...
  3. Format a prompt combining the context and the new input.
  4. Call the LLM.
...
`_format_prompt` is the method that builds the actual prompt string. **This is where the agent's personality lives.** Want the agent to talk differently? Change this method. Want it to use a system prompt instead of a single user message? Restructure here.

`MockLLM` is a deterministic fake LLM for the demo. Real use plugs in a function from `llm_clients.py`.

**When you'd touch this file:** when you want to change how turns work. Change the prompt to alter personality. Change `turn()` to add tool use, multi-step reasoning, or actions before responding. Add a new method like `reflect()` or `summarize()` for periodic background processing.

---

## llm_clients.py — Provider abstraction

**Responsibility:** provide a consistent `str -> str` callable interface to any LLM provider, with retries, error handling, and sensible defaults.
...
Four builder functions, all returning the same callable shape:
...
- Returns a callable that takes a prompt string and returns response text.
...
**Responsibility:** wire everything together, manage the persistent storage location, run the REPL.

**Knows about:** all the other modules. Where data lives. Which provider to use. The founding commitments.
...
- `FOUNDING_COMMITMENTS` — committed at genesis, immutable thereafter.
...
- `run()` — the REPL. Loads everything, commits genesis if needed, loops on input, dispatches commands or turns.
...
**When you'd touch this file:** all the time. This is your knob panel. Change `LLM_PROVIDER` to swap models. Change `SEMANTIC_K` and `RECENT_N` to tune retrieval. Change `FOUNDING_COMMITMENTS` *only on first run* — they're sealed at genesis and can't be changed without starting a new chain.
...
You type: "What did I tell you about apples?"
       ...
       agent.py
           ...
           │       │
           │       └─► retrieval.py: embed query, find similar
           │           records, blend with recent records
           │
...
Every turn writes two records: one for what you said, one for what the agent said. The agent's record references the records that informed it, so you have an auditable provenance trail: this response was derived from these specific prior records.
...
| The agent's personality / tone | `agent.py` — `_format_prompt` method |
| How much context the agent sees | `run.py` — `SEMANTIC_K` and `RECENT_N` |
| How retrieval works | `retrieval.py` — `Retriever` methods |
| Where data is stored | `run.py` — `DATA_DIR` |
| What the agent commits to at genesis | `run.py` — `FOUNDING_COMMITMENTS` (first run only) |
| The record schema | `chain.py` — `Record` dataclass + SCHEMA |
| Cryptographic primitives | `chain.py` — top of file |
| Adding tool use or actions | `agent.py` — extend `turn()` |
| Adding a new LLM provider | `llm_clients.py` — add a new `make_X_client()` |

---

## Two things worth understanding deeply
...
**Genesis commitments are permanent.** The founding commitments sealed at record 0 are part of the chain's identity. Changing them means starting a new chain. This is intentional: an agent whose founding commitments could be silently rewritten wouldn't have an identity, just a current configuration. The constraint enforces that what the agent committed to at the start remains auditable forever.
