...
└─────────────────────────────────────────────────────────┘
  ...
  llm_clients.py    PLUGGABLE — LLM provider clients
                    ...
                    with optional system prompt support
```
...
- `turn(user_input, retrieve_k)` — the heart of the loop:
  ...
  3. Format a prompt combining current time, retrieved context (with relative-time labels), any revisions targeting retrieved records, and the new input.
...
- `run()` — the REPL. Loads everything, commits genesis if needed, logs system prompt if changed, loops on input, dispatches commands or turns, auto-reflects on cadence.
